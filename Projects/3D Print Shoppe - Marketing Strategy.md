---
created: '2025-12-05T17:13:45.173169'
modified: '2025-12-05T17:13:45.173169'
privacy_scan: not_scanned
published_to_garden: false
tags: []
title: 3D Print Shoppe   Marketing Strategy
visibility: private
---

# 3D Print Shoppe - Marketing Strategy

## Platform Priority Stack

| Rank | Platform | Why | Effort | Status |
|------|----------|-----|--------|--------|
| 1 | Nextdoor | Hyper-local, exact target audience (WFH homeowners) | Low | Ready to launch |
| 2 | NTB Display | Passive lead gen, car enthusiast audience | Low | Active |
| 3 | Instagram | Visual products, Reels for organic reach | Medium | Winter break |
| 4 | Facebook Marketplace/Groups | Local buy/sell, community groups | Low | Post-launch |
| 5 | TikTok | Only if making videos (timelapses, reveals) | High | Maybe later |

**Skip:** LinkedIn (B2B, not my market), Twitter/X (not local-focused), Pinterest (low conversion for custom work)

---

## Nextdoor Strategy

### Optimal Timing
- **Best days:** Thursday or Friday
- **Best time:** 5-7pm local (WFH folks winding down)
- **Frequency:** Every 2 weeks (engagement dies after ~14 days)

### Posting Schedule
| Week | Post | Angle |
|------|------|-------|
| 1 (Launch) | Shoji Lamp | "Student pivot" - honesty about being new |
| 3 | Shoe Rack | "WFH problem solver" - functional utility |
| 5 | Great Wave Panel | "Local access" - art/hueforge capability |
| 7+ | Rotate new products | Keep it fresh |

### Tone Guidelines
- Sound like typing on phone after dinner
- Slightly rambling, personal, natural
- End with question or invitation to reply
- NO exclamation spam
- NO corporate language

---

## Nextdoor Posts (Ready to Use)

### Post 1: Thursday Launch - Shoji Lamp

```
hey neighbors - been lurking here for a while but finally have something worth sharing

I've been doing 3D printing from my place for a few months now, mostly just figuring things out. finally got a real website up and sorted out the boring legal stuff so figured I'd put myself out there

I'm Aim, GMU student, trying to help pay tuition doing something I actually enjoy. I make stuff like lamps, desk organizers, vases, that kind of thing. Not the flimsy plastic toys you're probably picturing - these come out with a matte finish that honestly looks more like ceramic or wood

this lamp is one of my favorites. Japanese-style, warm glow, fits on a nightstand or desk. took about 20 hours to print

anyway if you've got something you've been wanting to organize or decorate, or just curious what modern 3D printing can actually do - website is aim3dprints.com, or just message me here

no shipping, just porch pickup whenever works for you

- Aim
```

**Photo:** Shoji Lamp, lit

---

### Post 2: Week 3 - Shoe Rack

```
ok real talk - does anyone else's entryway look like a shoe explosion? just me?

I printed these modular rack things that stack however you want them. each one holds a couple pairs and they link together. finally got the mess by my front door under control

if you've got a weird corner or awkward space that store-bought organizers don't fit, that's kind of my thing. I can usually find or adapt a design in a day or two

$15 per rack piece, porch pickup. aim3dprints.com or DM me

- Aim
```

**Photo:** Shoe rack in use with real shoes

---

### Post 3: Week 5 - Hueforge Art

```
so this is a thing you can do now apparently

it's called hueforge - you take any image and recreate it in layered plastic. each layer catches light differently so you get this almost lithograph/relief carving effect

this one's the famous Great Wave but I've done car photos, pets, all kinds of stuff. makes a cool gift if you've got a family photo or something meaningful you want to turn into wall art

takes a while to print but the result is pretty unique. definitely not something you'll see at target

aim3dprints.com - Aim
```

**Photo:** Great Wave panel held at angle to show depth

---

## NTB Passive Lead Gen

### Setup
- Display prints at NTB Telegraph Rd (dad's shop)
- Gear shifter keychains for car guys
- 3D printed business cards
- QR code linking to aim3dprints.com

### Positioning
Don't say: "come here for pickup"
Do say: "browse my display at NTB Telegraph Rd next time you're getting tires"

### Products for Display
- Gear shifter keychains (car audience)
- Delorean model
- Business cards with keychain attached

---

## Business Card Strategy

### Format
Paper card (Avery 8371 cardstock) + mini keychain print combo

### Card Content
- Name: Aim
- Business: 3D Prints
- Phone: 571-866-3024 (text only)
- QR code to aim3dprints.com
- Tagline: "Your neighbor with a printer"

### Production
- Print on Epson WF-2950
- 80-110lb cardstock
- Hole punch corner, attach keychain with string/zip tie
- Cost: ~$0.50 each

---

## Testimonials

### Real (from Ryan L.)
> "The Buddha statue came out really nice - shiny finish, solid quality. Aim was super helpful with color options and even sent photos before I picked it up." - Ryan L., Lake Ridge

### Synthetic (approved for use)
> "Needed a headphone stand that didn't look like cheap Amazon junk. Aim printed one that matches my desk setup perfectly. Took like 3 days." - Marcus T.

> "Got a custom lamp for my daughter's room. He texted me when it was ready and I just grabbed it off his porch. Easiest transaction ever." - Jennifer K.

---

## Future Channels

### Instagram (Winter Break)
- Handle: @aims3dprintshoppe (check availability)
- Bio: "3D printed home decor and functional pieces | Lake Ridge, VA | No shipping fees | DM or aim3dprints.com"
- Content: Product shots, behind-the-scenes printer shots, customer pickups

### Launch Sequence
| Day | Post |
|-----|------|
| 1 | "Opening soon" teaser - Shoji Lamp |
| 2 | "Now accepting orders" - Vase |
| 3 | Great Wave close-up |
| 4 | Shoe rack in use |
| 5 | Behind-the-scenes printer shot |

---

## Engagement Rules

- Respond to every Nextdoor comment within 1 hour of posting
- Every comment bumps visibility in feed
- Get 5+ Nextdoor recommendations from friends/family (30% higher engagement)
- Never sound defensive, always helpful

---

## Related Notes
- [[Project - Aim's 3D Print Shoppe - Current State]]
- [[3D Print Shoppe - Session Notes 2024-12-05]]