---
tags: [penthouse, timeline, changelog, rebuild, website-rebuild]
created: 2026-03-05
...
## 2026-08-09 - v4.3.0-alpha.2 trust and safety release

Highlights:

- Deployed private member blocking, symmetric read-only blocked DMs, direct-contact enforcement, and blocked-pair presence, typing, and notification suppression.
- Replaced transient sidebar removal with a persistent personal DM archive that keeps the original chat and message history.
- Hardened final database permission checks and authenticated HTTP/socket rate limits.
- Tagged and deployed `v4.3.0-alpha.2` at revision `f03f3d0ca467a7003fd0ddd0f98b0d4e98c0427f`.
- Restored a pre-deploy database dump in isolation before cutover, applied migration `043_user_blocks.sql`, and retained rollback API/static/database artifacts.
- Completed a 14-check two-account production smoke and removed all disposable fixtures afterward.

Current boundaries:

- Provider-side push already in flight cannot be recalled.
- Presence can briefly reflect an older in-flight broadcast.
- Typing suppression remains single-instance until shared state is introduced.
- Dependency audit findings need a separate reviewed upgrade ticket.
